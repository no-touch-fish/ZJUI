...
def utility_partials(R, x):
    ...
    partial = np.zeros(2)
    partial[0] = dsig2(sig2(x[0])) @ R[0,:,:] @ sig2(x[1])
    # print('shape is',(sig2(x[0]) @ R[0,:,:]).shape)
    # print('another shape is',dsig2(sig2(x[1])).shape)
    partial[1] = sig2(x[0]) @ R[1,:,:] @ dsig2(sig2(x[1]))
    return partial
...
def episodic_game_gradient_ascent(init, rewards, nsteps, learningrate):
    ...
    for t in range(nsteps):
        for i in range(2):
            utilities[t][i] = sig2(logits[t,0])@rewards[i,:,:]@sig2(logits[t,1])
            if t+1 < nsteps:
                logits[t+1,i] = logits[t,i] + learningrate * utility_partials(rewards, logits[t,:])[i]
...
def utility_hessian(R, x):
    ...
    hessian[0][0] = Hsig2(sig2(x[0])) @ R[0,:,:] @ sig2(x[1])
    hessian[0][1] = dsig2(sig2(x[0])) @ R[1,:,:] @ dsig2(sig2(x[1]))
    hessian[1][0] = dsig2(sig2(x[0])) @ R[0,:,:] @ dsig2(sig2(x[1]))
    hessian[1][1] = sig2(x[0]) @ R[1,:,:] @ Hsig2(sig2(x[1]))
    return hessian
...
def episodic_game_corrected_ascent(init, rewards, nsteps, learningrate):
    ...
    for t in range(nsteps):
        for i in range(2):
            ...
            if t+1 < nsteps:
                # print('I is',np.diag((hessian.shape)))
                logits[t+1,i] = logits[t,i] + learningrate * ((np.eye(2) + symplectic_correction(partials, hessian))@partials)[i]
    return logits, utilities
...
'''
sequential_strategy = 0.5*np.ones((2,2))
sequential_strategy[0,0] = 0
sequential_strategy[0,1] = 0
sequential_strategy[1,0] = 1
